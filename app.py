...
def load_data_from_db():
    ...
    try:
        ...
        # 데이터베이스에 테이블이 없는 초기 상태일 경우 오류 대신 경고 표시
        st.warning(f"데이터를 불러오는 데 실패했습니다. 관리자 페이지에서 데이터를 먼저 업로드해주세요.")
        return None

# 데이터 로딩 시도
df = load_data_from_db()

# --- <<< 오류 수정: 데이터가 성공적으로 로드된 경우에만 전체 대시보드 표시 >>> ---
if df is not None and not df.empty:
    ...
    st.info("표시할 데이터가 없습니다. 관리자 페이지에서 데이터를 업로드해주세요.")
